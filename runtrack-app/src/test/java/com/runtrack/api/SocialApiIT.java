...
class SocialApiIT extends ApiIntegrationTest {
    ...
    void followingAPublicAccountIsImmediate() throws Exception {
        ...
        mvc.perform(get("/user/v1/" + marie.id() + "/followers").header("Authorization", bearer(marie)))
                .andExpect(status().isOk())
    ...
    void unfollowingRemovesTheLink() throws Exception {
        ...
        mvc.perform(delete("/user/v1/" + marie.id() + "/follow").header("Authorization", bearer(paul)))
    ...
    void nobodyFollowsThemselves() throws Exception {
        ...
        mvc.perform(post("/user/v1/" + marie.id() + "/follow").header("Authorization", bearer(marie)))
    ...
    void followingAnUnknownAccountIsNotFound() throws Exception {
        ...
        mvc.perform(post("/user/v1/018f4c1e-0000-7000-8000-0000000000ff/follow")
    ...
    void blockingBreaksTheFollowAndPreventsANewOne() throws Exception {
        ...
        mvc.perform(post("/user/v1/" + paul.id() + "/follow").header("Authorization", bearer(marie)))
                .andExpect(status().isForbidden());
    }
    ...
    void unblockingDoesNotRestoreTheFollow() throws Exception {
        ...
        mvc.perform(delete("/user/v1/" + paul.id() + "/block").header("Authorization", bearer(marie)))
                .andExpect(status().isNoContent());
    ...
    void nobodyBlocksThemselves() throws Exception {
        ...
        mvc.perform(post("/user/v1/" + marie.id() + "/block").header("Authorization", bearer(marie)))
                .andExpect(status().isConflict())
    ...
    void followRequestsAreListedForTheAuthenticatedUser() throws Exception {
        ...
        mvc.perform(get("/user/v1/me/follow-requests").header("Authorization", bearer(zoe)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray());

        mvc.perform(get("/user/v1/me/follow-requests")).andExpect(status().isUnauthorized());
    }
}
