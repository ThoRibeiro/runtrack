...
class SseSubscriberTest {
    ...
    private static PublishedEvent position(int sequence) {
        return new PublishedEvent("1700000000000-" + sequence, "position",
                "{\"sequenceNumber\":" + sequence + "}");
    }

    private static PublishedEvent snapshot(String payload) {
        return PublishedEvent.withoutId("status", payload);
    }

    @Test
    void sendsInTheOrderItReceived() throws Exception {
        var emitter = new RecordingSseEmitter();
        var subscriber = SseSubscriber.attachedTo(emitter, 10);
        emitter.expecting(3);
    ...
    void aFullQueueRefusesInsteadOfBlocking() {
        var subscriber = SseSubscriber.attachedTo(new RecordingSseEmitter(), 2);
